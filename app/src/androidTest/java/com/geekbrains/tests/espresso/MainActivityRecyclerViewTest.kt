...
class MainActivityRecyclerViewTest {
    ...
    fun activitySearch_ScrollTo() {
        if (BuildConfig.TYPE == MainActivity.FAKE) {
            ...
            onView(withId(R.id.searchEditText)).perform(replaceText("android"), closeSoftKeyboard())
            onView(withId(R.id.searchEditText)).perform(pressImeActionButton())

            onView(isRoot()).perform(delay())
            onView(withId(R.id.recyclerView)).perform(RecyclerViewActions.scrollTo<SearchResultViewHolder>(
                        hasDescendant(withText("surespot/android"))
                    )
                )
        }
    }
    ...
    fun activitySearch_PerformClickAtPosition() {
        if (BuildConfig.TYPE == MainActivity.FAKE) {
            ...
            onView(isRoot()).perform(delay())
            onView(withId(R.id.recyclerView)).perform(RecyclerViewActions.actionOnItemAtPosition<SearchResultViewHolder>(
                        0, click()
                    )
                )
        }
    ...
    fun activitySearch_PerformClickOnItem() {
        if (BuildConfig.TYPE == MainActivity.FAKE) {
            ...
            onView(isRoot()).perform(delay())
            onView(withId(R.id.recyclerView)).perform(RecyclerViewActions.actionOnItem<SearchResultViewHolder>(
                        hasDescendant(withText("surespot/android")), click()
                    )
                )
        }
    }
    ...
    fun activitySearch_PerformCustomClick() {
        if (BuildConfig.TYPE == MainActivity.FAKE) {
            ...
            onView(isRoot()).perform(delay())
            onView(withId(R.id.recyclerView)).perform(RecyclerViewActions.actionOnItemAtPosition<SearchResultViewHolder>(
                            8, tapOnItemWithId(R.id.checkbox))
                )
        }
    }
